...
- **[helper_ai.py](shared/mode_configs.py)** (Single Source of Truth for AI Modes)
...
- **[config.py](shared/config.py)**: Environment detection and LLM configuration
  - `where_am_i()`: Detects PROD vs Local environment
  - LLM provider and model settings

- **[llm_provider.py](shared/llm_provider.py)**: LLM abstraction layer
  - `GeminiProvider`: Production LLM (Google Gemini API)
  - `OllamaProvider`: Local development only

- **[helper_db.py](shared/helper_db.py)**: Database operations with automatic environment detection
...
**Authentication Router** ([routers/auth.py](fastapi_app/routers/auth.py)):
...
**Text Improvement Router** ([routers/text.py](fastapi_app/routers/text.py)):
...
**Statistics Router** ([routers/stats.py](fastapi_app/routers/stats.py)):
...
1. Python defines modes in [shared/helper_ai.py](shared/mode_configs.py) (single source of truth)
...
3. `pnpm generate-api` does two things:
   ...
   - Runs [scripts/gen_mode_descriptions.py](scripts/gen_mode_descriptions.py) to extract mode descriptions
4. Frontend uses auto-generated types and descriptions

## Features

see **Available Modes** (defined in [shared/helper_ai.py](shared/mode_configs.py)):
...
For local development and testing. Set `LLM_PROVIDERS=Mock` in `.env`. Returns static responses without calling any external API.
...
Secret: MISTRAL_API_KEY from <https://console.mistral.ai/keys>

### OpenAI LLM

Secret: OPENAI_API_KEY from <https://platform.openai.com/settings/organization/api-keys>
