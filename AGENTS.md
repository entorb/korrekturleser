...
Each `chk_*.sh` prints `OK` on success (no errors found), or failure output + non-zero exit.
...
Issues: `https://sonarcloud.io/project/issues?id=entorb_korrekturleser`
API fetch: `curl "https://sonarcloud.io/api/issues/search?componentKeys=entorb_korrekturleser&ps=500" | jq '.'`
