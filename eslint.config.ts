...
export default [
  ...
  // Base TypeScript files
  {
    ...
    rules: {
      ...
      ...(sonarjs.configs?.recommended && 'rules' in sonarjs.configs.recommended
  ...
  // Vue composables - relax some rules
  {
    files: [
      ...
      'packages/shared/src/composables/**/*.ts',
      'vue_app/src/composables/**/*.ts'
    ],
    rules: {
  ...
  // Playwright files - relaxed rules
  {
    files: ['e2e/**/*.{ts,tsx}'],
    languageOptions: {
      parser: tsParser,
    ...
    rules: {
      // Playwright tests use their own test/expect, not Vitest's
      'vitest/*': 'off',
